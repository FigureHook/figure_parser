...
class TestProductTextUtils:
    def test_general_attribute_normalization(self):
        ...
        assert general_normalize(text_should_be_half_width) == "KADOKAWA"
        assert general_normalize(text_with_duplicate_space) == "too much spaces Ver."
        assert general_normalize(text_with_weird_quotation) == "hello 'there'"
    ...
    def test_worker_attribute_normalization(self):
        ...
        assert worker_normalize(text_with_full_width_brackets) == "(Hello, there)"
        assert worker_normalize(text_with_no_space_before_bracket) == "Master (HW)"
        assert (
            worker_normalize(text_with_no_space_before_square_bracket) == "Newbie (NW)"
        )
